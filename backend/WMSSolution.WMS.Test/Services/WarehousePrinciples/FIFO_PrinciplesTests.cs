...
[TestClass]
public class FIFO_PrinciplesTests: WarehousePrinciplesTests
{
    [TestInitialize]
    public void Setup()
    {        
        _principles = new WarehouseManagementPrinciples(_inventory, new FifoRetrievalStrategy());
    }

    [TestMethod]
    public void GetItemsByCategory_FIFO_ReturnList()
    {
        ...
        var items = _inventory.Where(x => x.Category == _category)
            .OrderBy(i => i.ReceivedDate).ToList();
    ...
    [TestMethod]
    public void GetItemsByQuantity_FIFO_ReturnList()
    {
        // Arrange - No data in database
        // Act
    ...
    [TestMethod]
    public void GetItems_FIFO_ReturnList()
    {
        // Arrange - No data in database
        // Act
        int qty = random.Next(23, 100);
