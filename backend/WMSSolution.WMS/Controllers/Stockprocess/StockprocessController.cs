...
namespace WMSSolution.WMS.Controllers.Stockprocess;
...
public class StockprocessController(IStockprocessService stockprocessService
      , IStringLocalizer<MultiLanguage> stringLocalizer) : BaseController
{
    ...
    {
        var (data, totals) = await _stockprocessService.PageAsync(pageSearch);
    ...
    {
        StockprocessWithDetailViewModel? data = await _stockprocessService.GetAsync(id);
        if (data != null)
        {
    ...
    public async Task<ResultModel<StockProcessDashboardStatsViewModel>> GetDashboardStats()
    {
        var data = await _stockprocessService.GetDashboardStatsAsync();
        return ResultModel<StockProcessDashboardStatsViewModel>.Success(data);
    }
