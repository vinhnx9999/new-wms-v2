...
namespace WMSSolution.WMS.Controllers
{
    ...
    public class DispatchlistController : BaseController
    {
        ...
        /// <summary>
        /// Creates a dispatch order in draft status.
        /// The order can be reviewed and modified before execution.
        /// </summary>
        /// <param name="request">The dispatch order request</param>
        /// <returns>Result with dispatch ID</returns>
        [HttpPost("draft")]
        public async Task<ResultModel<int>> CreateDispatchDraft([FromBody] CreateDispatchOrderRequest request)
        {
            var (dispatchId, message) = await _dispatchlistService.CreateDispatchDraftAsync(request);
            if (dispatchId > 0)
            {
                return ResultModel<int>.Success(dispatchId);
            }
            else
            {
        ...
        /// <summary>
        /// Executes a draft dispatch order by changing its status to Processing
        /// and creating outbound tasks for WCS integration.
        /// </summary>
        /// <param name="id">The dispatch header ID to execute</param>
        /// <returns>Result indicating success or failure</returns>
        [HttpPut("execute/{id}")]
        public async Task<ResultModel<string>> ExecuteDispatch(int id)
        {
            var success = await _dispatchlistService.ExecuteDispatchAsync(id, CurrentUser);
            if (success)
            {
                return ResultModel<string>.Success(_stringLocalizer["operation_success"]);
            }
            else
            {
                return ResultModel<string>.Error(_stringLocalizer["operation_failed"]);
            }
        }

        /// <summary>
        /// Creates and immediately executes a dispatch order in a single atomic operation.
        /// Skips the draft review step.
        /// </summary>
        /// <param name="request">The dispatch order request</param>
        /// <returns>Result with dispatch ID</returns>
        [HttpPost("create-and-execute")]
        public async Task<ResultModel<int>> CreateAndExecuteDispatch([FromBody] CreateDispatchOrderRequest request)
        {
            var (dispatchId, success, message) = await _dispatchlistService.CreateAndExecuteDispatchAsync(request, CurrentUser);
            if (success)
            {
                return ResultModel<int>.Success(dispatchId);
            }
