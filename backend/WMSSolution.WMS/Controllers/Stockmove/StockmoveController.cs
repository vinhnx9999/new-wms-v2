...
namespace WMSSolution.WMS.Controllers
{
    ...
    public class StockmoveController : BaseController
    {
        ...
        /// <returns></returns>
        [HttpPut("confirm")]
        public async Task<ResultModel<bool>> Confirm([FromQuery] int id)
        {
        ...
        public async Task<ResultModel<StockMoveDashboardStats>> GetDashboardStatsAsync()
        {
            var data = await _stockmoveService.GetDashboardStatsAsync(CurrentUser);
            return ResultModel<StockMoveDashboardStats>.Success(data);
        }
        #endregion
