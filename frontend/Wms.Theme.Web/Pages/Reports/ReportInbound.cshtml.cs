...
public class ReportInboundModel(IReportService service,
    ...
    IWarehouseService warehouseService) : PageModel
{
    ...
    public IEnumerable<ImportReportItem> ReportItems { get; set; } = [];
    public async Task OnGetAsync()
    {
        ...
        ReportItems = await _service.GetReportIncomingGoods(request);
    }

    public async Task<IActionResult> OnPostSearchData([FromBody] InventoryReportRequest request)
    {
        var items = await _service.GetReportIncomingGoods(request);
        ReportItems = items;
