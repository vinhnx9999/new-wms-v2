...
public class ReportOutboundModel(IReportService service,
    ...
    IWarehouseService warehouseService) : PageModel
{
    ...
    public IEnumerable<ExportReportItem> ReportItems { get; set; } = [];
    public async Task OnGetAsync()
    {
        ...
        ReportItems = await _service.GetReportOutgoingGoods(request);
    }

    public async Task<IActionResult> OnPostSearchData([FromBody] InventoryReportRequest request)
    {
        var items = await _service.GetReportOutgoingGoods(request);
        ReportItems = items;
