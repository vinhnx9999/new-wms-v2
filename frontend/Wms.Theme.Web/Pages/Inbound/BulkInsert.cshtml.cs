...
public class BulkInsertModel(IWarehouseService warehouseService,
                     ...
                     IPurchaseOrderService purchaseOrderService) : PageModel
{
    ...
    public async Task<JsonResult> OnGetSearchSku(int supplierId, 
        string? keyWord, int pageIndex = SystemConfig.DEFAULT_INDEX, int pageSize = SystemConfig.PAGE_SIZE)
    {
        var searches = new List<SearchObject>
    {
            new() {
    ...
    public async Task<JsonResult> OnPost([FromBody] CreateBulkReceiptRequest request)
    {
        if (request is null)
        {
        ...
        var (id, message) = await _receiptService.CreateNewBulkReceiptAsync(request);
        if (id == 0)
        {
    ...
    public async Task<JsonResult> OnGetSearchSupplier(string? keyword, int pageIndex = SystemConfig.DEFAULT_INDEX)
    {
        var searches = new List<SearchObject>
    {
    ...
    public async Task<JsonResult> OnPostCalculatorPallets([FromBody] CalculatorPalletRequest request)
    {
        if (request is null)
        {
            return new JsonResult(new { success = false, message = "Invalid request data" });
        ...
        var data = await _warehouseService.CalculatorPallets(request);
        return new JsonResult(data);
    }
